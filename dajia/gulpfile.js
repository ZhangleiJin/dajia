var gulp = require('gulp'),
	...
	scriptConfig = require('./sprite.config.js'),
    ...
    sprite = require('gulp-css-spritesmith'),//自动雪碧图
...
// concats
var concatFuns = '';
for(var c_i in config){
	concatFuns += 'gulp.src(config['+c_i+'].from).on("error", swallowError).pipe(concat(config[' + c_i + '].to)).on("error", swallowError)'+`.pipe(rev())
...
var tasks = {
	clean:function(){
		gulp.src(['public/','view/'],{read:false})
			.on('error', swallowError)
			.pipe(clean())
		;
	},
	...
	sprite:function(){
		gulp.src(filePaths.cssPublicSrc)
			.on('error', swallowError)
			.pipe(sprite(scriptConfig))
			.on('error', swallowError)
			.pipe(gulp.dest('./'))
		;
	},
	revjs:function(){
		// gulp.src(config[0].from)
		// 	.on('error', swallowError)
		// 	.pipe(concat(config[0].to))
		// 	.on('error', swallowError)
		// 	// .pipe(uglify())
		// 	.pipe(rev())
		// 	.on('error', swallowError)
...
//css替换背景图版本2
gulp.task('revcssimgclean', function () {
    return tasks.revcssimg();
});
...
// 上线状态
gulp.task('product',['min'],function(){
	setTimeout(function(){
		return tasks.sprite();
	},500);
});
...
//     tasks.min();
// });
// gulp.task('product',['min','sprite'],function(){
// 	console.log('done!');
// });
    	...
    	if(e.type == 'deleted'){
    		...
    		if(isAssets){
	    		if(isJs){
	    			...
	    			gulp.start('revcssimgclean');
	    			gulp.start('revhtml');
	    		}else{
	    			gulp.start(['revcssimg']);
	    		}
	    	}else{
	    		gulp.start(['revhtml']);
	    	}
